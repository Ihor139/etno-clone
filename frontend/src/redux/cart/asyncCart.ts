...
export const fetchCart = createAsyncThunk("getCart", async () => {
	const {data} = await axios.get<CartGroup>(process.env.BASE_URL+"/cart");
	return data;
});

export const addToCart = createAsyncThunk("addToCart", async (product: CartItemNew) => {
	const {data} = await axios.post<CartGroup>(process.env.BASE_URL+"/cart", product);
	return data;
});

export const removeFromCart = createAsyncThunk("removeFromCart", async (product: { _id: string }) => {
	const {data} = await axios.post<CartGroup>(process.env.BASE_URL+"/cart/remove", product);
	return data;
});

export const updateQuantity = createAsyncThunk("updateQuantity", async (product: { _id: string, amount: number }) => {
	const {data} = await axios.post<CartGroup>(process.env.BASE_URL+"/cart/update", product);
	return data;
});
